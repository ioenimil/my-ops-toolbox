...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--include-regex",
        metavar="REGEX",
        dest="include_regexes",
        action="append",
        default=[],
        help="Include repositories whose names match REGEX. Repeatable.",
    )
    parser.add_argument(
        "--filter-config",
        metavar="PATH",
...
def main() -> None:
    ...
    results = asyncio.run(
        sync_organisation(
            ...
            token=token,
            rules=rules,
            prefix_includes=args.include_prefixes,
            suffix_includes=args.include_suffixes,
            regex_includes=args.include_regexes,
